...
router.post('/create', async (req, res) => {
  ...
  try {
    ...
    if (
      !Studiengangsname ||
      !Array.isArray(SemesterModule) ||
      SemesterModule.length !== 6
    ) {
      return res
        .status(400)
        .json({
          message:
            'Studiengangsname and SemesterModule (6 arrays) are required!',
        });
    }
  ...
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({
          message: 'Studiengangsname und 6 Semester-Module sind erforderlich!',
        });
    }
  ...
  try {
    ...
    if (result.deletedCount > 0) {
      ...
      return res
        ...
        .json({
          success: true,
          message: `${result.deletedCount} SG wurden erfolgreich gelöscht.`,
        });
    } else {
      return res
        .status(404)
