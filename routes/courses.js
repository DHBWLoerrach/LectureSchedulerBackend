...
router.post('/calendar/update', async (req, res) => {
  try {
    const { courseId, Kalender, user, crs, module, userLevel, userID } = req.body;
    ...
    for (const key in Kalender) {
      if (Kalender.hasOwnProperty(key)) {
        if (currentKalender.hasOwnProperty(key) && currentKalender[key] !== Kalender[key]) {
    ...
    const isUserAssigned =
      ...
      currentCourse.ZugewiesenSekretariat.some(assignedId => assignedId.toString() === userID);
    ...
    res.status(500).json({ success: false, message: 'Error updating calendar' });
  }
});
...
router.post('/create', async (req, res) => {
  ...
  try {
    if (!Kursname || !Studiengang || !Semester) {
      return res.status(400).json({ message: "Modulname and Vorlesungsstunden are required!" });
    }
...
router.put('/edit', async (req, res) => {
  const { id, Kursname, Studiengang, Semester, ZugewieseneSekretariat } = req.body;
  console.log(id, Kursname, Studiengang, Semester, ZugewieseneSekretariat);

  try {
    // Überprüfen, ob alle erforderlichen Felder vorhanden sind
    if (!id || !Kursname || !Studiengang || !Semester) {
      return res.status(400).json({ message: "ID, Modulname, Studiengang und Semester sind erforderlich!" });
    }

    // Konvertiere die string IDs zu MongoDB ObjectIds
    const validDozenten = Array.isArray(ZugewieseneSekretariat)
      ? ZugewieseneSekretariat
          .filter(id => mongoose.Types.ObjectId.isValid(id)) // Filtere ungültige IDs heraus
          .map(id => new mongoose.Types.ObjectId(id))
      : [];
  ...
  try {
      ...
      if (result.deletedCount > 0) {
        ...
        */
          ...
          return res.status(200).json({ success: true, message: `${result.deletedCount} Course wurden erfolgreich gelöscht.` });
      } else {
