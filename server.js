...
const mongoUrlPersonen = process.env.MONGO_URL || "mongodb://localhost:27017/Personen";
...
const authenticateToken = async (req, res, next) => {
    ...
    try {
        ...
        const detailsMatch = dbUser.Benutzername === decoded.username &&
...
app.get('/audit/get', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Error fetching audit trail' });
  }
});
...
app.post('/employees/create', authenticateToken, async (req, res) => {
  try {
      ...
      if (existingEmployee) {
          return res.status(400).json({ success: false, message: 'Employee with this Exyte Email already exists' });
      }
...
app.post('/employees/edit', authenticateToken, async (req, res) => {
  try {
      ...
      if (!existingEmployee) {
          return res.status(404).json({ success: false, message: 'Employee with this ID does not exist' });
      }

      // Check if the new SerialNumber already exists for a different employee (except where the id is the same)
      const employeeWithSameEmail = await User.findOne({ username, _id: { $ne: employeeID } });
  ...
  try {
      ...
      if (result.deletedCount > 0) {
        ...
        */
          ...
          return res.status(200).json({ success: true, message: `${result.deletedCount} Mitarbeiter wurden erfolgreich gelöscht.` });
      } else {
          return res.status(404).json({ success: false, message: 'Keinen Mitarbeiter gefunden.' });
      }
